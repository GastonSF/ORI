...
export default async function OfficerInbox() {
  const supabase = await createClient()

  // El oficial ve todos los legajos - RLS ya filtra correctamente
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <header className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Bandeja</h1>
        <p className="mt-1 text-sm text-gray-600">
          Legajos que requieren tu atención.
        </p>
      </header>

      {!apps || apps.length === 0 ? (
        <div className="rounded-lg border border-gray-200 bg-white p-10 text-center text-sm text-gray-600">
          No hay legajos pendientes en este momento. 🎉
        </div>
      ) : (
              ...
              {apps.map((a) => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-block rounded-full bg-blue-50 text-[#1b38e8] px-2.5 py-0.5 text-xs font-medium">
